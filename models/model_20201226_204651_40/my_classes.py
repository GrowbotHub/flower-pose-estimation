...
class RealFlowersDataset(torch.utils.data.Dataset):

    """Dataset with real flowers"""

    def __init__(self, file_names, root_dir, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        # Define bboxes in order to use already defined 
        # 'MyTransformation' transformation
        bbox_2d = np.array([1]).astype('float')
        bbox_3d = np.array([1]).astype('float')
...
class MyTransformation(object):
    ...
    def __init__(self,image_transforms=[
        ...
        twofiftyfive : bool
            If 'True' each image is will be in the range [0,255] instead
            of [0.0,1.0].
        """
