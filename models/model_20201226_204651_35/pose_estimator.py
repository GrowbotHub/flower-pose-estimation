        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                if epoch == 14 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_15") 
                if epoch == 19 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_20") 
                if epoch == 24 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_25")
                if epoch == 29 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_30")
                if epoch == 34 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_35")
                if epoch == 39 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_40")
                if epoch == 44 and phase == 'val':
                    torch.save(model.state_dict(), dirname+"/model_epoch_45")
...
### Train a new model
model = GilNet(model='resnet34',pretrained=True,num_outputs=16,out_func=lambda x: x)

model = model.to(device)
criterion = S_Loss_MSE

# Observe that all parameters are being optimized
optimizer_ft = optim.SGD(model.parameters(), lr=0.00008, momentum=0.9)
